...
| Synthetic LeRobot→RLDS E2E | **CI通過** | `tests/test_synthetic_rlds_e2e.py` |
| GitHub Actions | **2 Job通過実績あり** | `.github/workflows/ci.yml` |
| 提出requirements検査 | 実装・CI通過 | `scripts/validate_submission_requirements.py` |
| Checkpoint Manifest | 実装・Unit通過 | `build_checkpoint_manifest.py`、`checkpoint_manifest.schema.json` |
...
| Notebook・YAML・Shell検査 | CI通過 | `.github/workflows/ci.yml` |
